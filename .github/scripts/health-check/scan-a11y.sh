...
if [[ -n "$CLICK_ON_DIV" ]]; then
  ...
  if [[ -n "$NO_ROLE" ]]; then
    COUNT=$(echo "$NO_ROLE" | count_lines)
...
if [[ -n "$ICON_BUTTONS" ]]; then
  ...
  if [[ -n "$NO_LABEL" ]]; then
    COUNT=$(echo "$NO_LABEL" | count_lines)
...
if [[ -n "$INPUTS_NO_LABEL" ]]; then
  COUNT=$(echo "$INPUTS_NO_LABEL" | count_lines)
...
if [[ -n "$AUTOFOCUS" ]]; then
  COUNT=$(echo "$AUTOFOCUS" | count_lines)
...
if [[ -n "$EMPTY_LINKS" ]]; then
  ...
  if [[ -n "$NO_TEXT" ]]; then
    COUNT=$(echo "$NO_TEXT" | count_lines)
