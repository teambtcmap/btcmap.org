...
for chunk in chunks:
    ...
    piece = chunk
    if len(piece) > MAX_PER_FILE:
        piece = piece[:MAX_PER_FILE] + (
...
if truncated or omitted:
    manifest = "\n=== DIFF COVERAGE ===\n"
    if truncated:
        manifest += f"{truncated} file diff(s) above were truncated to {MAX_PER_FILE} bytes each.\n"
    if omitted:
        manifest += (
...
with open(f"{TMP}/pipeline-stats.txt", "w", encoding="utf-8") as f:
    f.write(
        ...
        f"(per-file cap {MAX_PER_FILE}); {truncated} file(s) truncated; "
