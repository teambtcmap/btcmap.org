...
describe("session store", () => {
	...
	describe("signUp with chosen credentials", () => {
		...
		it("sends the chosen username and password to the signup route", async () => {
			...
			await session.signUp("alice", "correct horse battery");
		...
		it("marks a session with chosen credentials as not auto-generated", async () => {
			...
			const current = await session.signUp("alice", "correct horse battery");
		...
		it("trims the username before sending it", async () => {
			...
			await session.signUp("  bob  ", "correct horse battery");
		...
		it("rejects a blank username when a password is given", async () => {
			const post = await postMock();
			const session = await createTestSession();

			await expect(
				session.signUp("   ", "correct horse battery"),
			).rejects.toThrow();
		...
		it("rejects a username without a password", async () => {
			...
			await expect(session.signUp("bob")).rejects.toThrow();
			expect(post).not.toHaveBeenCalled();
		});
		...
		it("does not coalesce an explicit signup with a pending throwaway", async () => {
			...
			const explicit = session.signUp("alice", "correct horse battery");
