...
function createSessionStore() {
	...
	const doSignUp = async (
		...
		const session: Session = {
			...
			savedPlaces: [],
			savedAreas: [],
			// Auto-generated only when the caller didn't supply a username —
			// the SaveAuthPrompt path keeps creating throwaways, while an
			// explicit /signup form (which provides both) records the user as
			// having chosen their own credentials.
	...
	return {
		...
		// still produces a throwaway (used by the SaveAuthPrompt flow). With
		// both username and password, the user picks their own credentials and
		// the resulting session is marked non-auto-generated.
		//
		...
		signUp: (username?: string, password?: string): Promise<Session> => {
			...
			signUpInFlight = doSignUp(username, password).finally(() => {
				signUpInFlight = null;
			});
