...
export const toggleMapButtons = () => {
	const zoomInBtn: HTMLAnchorElement | null = document.querySelector(
		".leaflet-control-zoom-in",
	);
	const zoomOutBtn: HTMLAnchorElement | null = document.querySelector(
	...
	const zoomOutImg: HTMLImageElement | null =
		document.querySelector("#zoomout");
	const fullScreenImg: HTMLImageElement | null =
		document.querySelector("#fullscreen");
	const locateImg: HTMLImageElement | null =
...
export const support = () => {
	...
	supportAttribution.style.filter =
...
export const changeDefaultIcons = (
	...
	if (layers) {
		const layers: HTMLDivElement | null = document.querySelector(
	...
	if (zoomIn) {
		...
		if (theme === "light") {
			const zoomInIcon: HTMLImageElement | null =
	...
	if (zoomOut) {
		...
		if (theme === "light") {
			const zoomOutIcon: HTMLImageElement | null =
				document.querySelector("#zoomout");
			if (zoomOutIcon) {
...
export const geolocate = (
	...
	newLocateIcon.src =
		theme === "dark" ? "/icons/locate-white.svg" : "/icons/locate.svg";
	newLocateIcon.alt = "locate";
	newLocateIcon.classList.add("inline");
	newLocateIcon.id = "locatebutton";
	document
		.querySelector(".leaflet-control-locate-location-arrow")
		?.replaceWith(newLocateIcon);

	const locateDiv: HTMLDivElement | null = document.querySelector(
	...
	if (locateButton) {
		...
		if (theme === "light") {
			const locateIcon: HTMLImageElement | null =
				document.querySelector("#locatebutton");
			if (locateIcon) {
			...
			if (mainMap) {
				...
				if (theme === "light") {
					merchantMapButton.onmouseenter = () => {
						const shoppingIcon = document.querySelector(
							"#shopping",
						) as HTMLImageElement;
						if (shoppingIcon) {
