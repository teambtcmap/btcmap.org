<script lang="ts">
	...
	// Calculate score function (same as original)
	...
	const columns: ColumnDef<LeaderboardArea>[] = [
		...
		},
		{
			...
			accessorFn: (row) => row.report.tags.total_elements,
			enableSorting: true
		},
		{
			id: 'grade',
		...
		},
		{
			id: 'score',
			header: 'Score',
			accessorFn: (row) => score(row.report),
			enableSorting: true
		}
	];

	// Writable stores for table state
	let data = writable<LeaderboardArea[]>([]);
	let sorting = writable([{ id: 'score', desc: true }]); // Default sort by score descending
...
<div class="p-4">
	...
	{:else}
		...
		<div class="mt-6 flex items-center justify-between">
			<div class="flex items-center gap-4">
				<button
					...
					class="rounded bg-primary px-4 py-2 text-white transition-colors hover:bg-hover disabled:cursor-not-allowed disabled:bg-gray-300"
				>
					Previous
				</button>
		...
		<!-- Additional info -->
		<div class="mt-4 text-center text-sm text-body dark:text-white">
