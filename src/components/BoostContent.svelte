...
{#if stage === 0}
  <div class="space-y-4">
    <div>
      <p class="mb-2 text-xl font-bold text-primary dark:text-white">
        Boost Location
      </p>

      <p class="text-sm text-body dark:text-white">
        Make this merchant stand out in bitcoin orange on the map, shine in the
        search results, and be discovered in the exclusive boosted locations
        map!
      </p>
          ...
          on:click={() => {
            ...
            selectedBoost = {
              ...
              expires: currentBoost
                ? new Date(
                    currentBoost.setMonth(currentBoost.getMonth() + value.time),
                  )
    ...
    <p class="text-xs text-body dark:text-white">
      The fee is used to support the BTC Map open source project and continue
      it's development.
    </p>

    <PrimaryButton
      style="w-full rounded-xl p-3 {!selectedBoost
...
{:else}
  <div class="space-y-4 text-center">
    <p
      class="text-xl font-bold text-primary dark:text-white {merchantName?.match(
        '([^ ]{14})',
      )
        ? 'break-all'
        : ''}"
