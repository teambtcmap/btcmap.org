...
const iconPaths = {
	btc: {
		...
		no: {
			light:
				variant === "teal" ? "/icons/btc-no-teal.svg" : "/icons/btc-no.svg",
	...
	nfc: {
		...
		no: {
			light:
				variant === "teal" ? "/icons/nfc-no-teal.svg" : "/icons/nfc-no.svg",
...
$: iconSrc =
	$theme === "dark"
		? iconPaths[method][statusKey].dark
		: iconPaths[method][statusKey].light;
