...
<div class="hidden lg:block" role="region" aria-label="Leaderboard table">
  <table
    class="w-full text-left text-xs text-primary lg:text-sm xl:text-lg dark:text-white"
  >
    <thead>
            ...
            >
              {#if cell.column.id === "name"}
                <AreaLeaderboardItemName
                  ...
                  id={cell.row.original.tags?.url_alias ||
              ...
              {:else if cell.column.id === "grade"}
                ...
                {@const percentage =
                  cell.row.original.report?.tags?.up_to_date_percent}
                {@const avgDate =
                  cell.row.original.report?.tags?.avg_verification_date}
