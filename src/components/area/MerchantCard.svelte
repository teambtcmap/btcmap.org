...
>
  <div>
    <div
      class="mb-3 flex w-full flex-col items-center justify-between gap-2 sm:flex-row"
    >
      <a
      ...
      >
        ...
        <p class="text-lg break-all">
          {displayMerchant.name || "BTC Map Merchant"}
        </p>
      </a>
    </div>
    ...
    <div class="mb-3 w-full space-y-2 break-all text-primary dark:text-white">
      ...
      <div class="flex items-center space-x-2">
        ...
        {#if instagram}
          <!-- eslint-disable svelte/no-navigation-without-resolve -->
          <a
            href={instagram.startsWith("http")
              ? instagram
  ...
  <div
    class="w-full space-y-2 border-t border-gray-300 pt-3 dark:border-gray-300/25"
  >
    {#if verified.length}
        ...
        <p class="text-sm font-semibold text-gray-500 dark:text-gray-400">
          Boost Expires: <span class="text-primary dark:text-white"
            ><Time
              live={3000}
              relative={true}
              timestamp={merchant.boosted_until}
            /></span
          >
        </p>
      </div>
    ...
    <div class="flex justify-between space-x-2 sm:justify-start">
      ...
      <BoostButton
        {merchant}
        boosted={boosted ? merchant.boosted_until : undefined}
        style="link"
      />
    </div>
  </div>
