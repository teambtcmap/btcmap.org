    ...
    >
      {#if mode === "search"}
        ...
        {#if isSearching || searchResults.length > 0 || searchQuery.length >= 3}
          <p
            class="mt-2 text-xs text-body dark:text-white/70"
            aria-live="polite"
          >
            {#if isSearching}
              Searching...
      ...
      {:else}
        ...
        <div class="flex items-center justify-between">
          <div>
            <h2 class="text-sm font-semibold text-primary dark:text-white">
              Nearby Merchants
            </h2>
            {#if showZoomInMessage}
              <p
                class="text-xs text-body dark:text-white/70"
                aria-live="polite"
              >
                Zoom in to see list
              </p>
            ...
            {:else}
              <p
                ...
                aria-live="polite"
              >
                {merchants.length} location{merchants.length !== 1 ? "s" : ""} in
                view
              </p>
            {/if}
          </div>
