...
{#if isOpen}
  ...
  >
    ...
    >
      <!-- Drag handle -->
      <div
        class="mx-auto mt-2 h-1.5 w-12 rounded-full bg-gray-300 dark:bg-white/30"
      ></div>

      <!-- Header -->
      <div
        class="flex items-center bg-white px-4 dark:bg-dark {drawerView ===
        'details'
          ? 'justify-end py-1'
      ...
      >
        {#if drawerView !== "details"}
          ...
          <span
            class="text-sm font-semibold text-primary capitalize dark:text-white"
            >{drawerView}</span
          >
        {/if}
