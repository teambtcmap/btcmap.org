...
<main class="my-10 space-y-10 text-center md:my-20">
  ...
  <h2
    class="mx-auto w-full text-xl font-semibold text-primary lg:w-[800px] dark:text-white"
  >
    Take ownership of your local bitcoin mapping data and help drive adoption.
    Bitcoin communities are the spark that ignites the movement. Join your
    friends, onboard businesses and have fun!
  </h2>
  ...
  <div>
    ...
    <div class="flex flex-col items-center justify-center gap-5 md:flex-row">
      <PrimaryButton
        style="md:w-[200px] py-3 rounded-xl w-full"
  ...
  <div class="relative">
    {#if !initialRenderComplete || !communities}
      <div
        class="absolute top-0 left-0 flex h-full w-full flex-col justify-between"
      >
        <div class="flex flex-wrap justify-center">
          {#each Array(6) as _, i (i)}
            <div
              class="m-2 w-[94px] animate-pulse rounded-sm bg-link/50 py-2"
            />
          {/each}
        </div>
        <div
      ...
      {#if data.section}
        ...
        >
          <a href={resolve(`/communities/${data.section}`)}>
            {organizationSections.find((org) => org.id === data.section)
              ?.displayName ||
        ...
        >
          {#each sections as option (option)}
            <option disabled={option.startsWith("--")} value={option}>
              {option.startsWith("--")
                ? option
                : organizationSections.find((org) => org.id === option)
                    ?.displayName ||
