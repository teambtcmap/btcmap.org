...
<svelte:head>
  ...
  <meta
    ...
    content="{name ? name + ' - ' : ''}BTC Map Merchant"
  />
  ...
  <div class="bg-red-600 py-4 text-center text-white">
    <p class="text-lg font-semibold">
      <Icon
        w="20"
        h="20"
        class="mr-2 inline-block text-white"
        icon="skull"
        type="material"
      />
    ...
    <p class="mt-1 text-sm">
      The data shown below is outdated and for reference only.
    </p>
  </div>
{/if}
<main class="my-10 space-y-16 text-center md:my-20">
  <section id="profile" class="space-y-8">
    <div class="space-y-2">
      {#if icon}
        <div
          class="mx-auto flex h-32 w-32 items-center justify-center rounded-full {data
            .placeData.deleted_at
      ...
      <h1
        class="text-4xl !leading-tight font-semibold text-primary dark:text-white"
      >
        ...
        <div class="text-primary dark:text-white">
          ...
          <div class="flex items-start justify-center">
            ...
            <time class="flex flex-col items-start"
              >{@html formatOpeningHours(hours)}</time
            >
          </div>
        </div>
      {/if}
        ...
        <MerchantButton
          on:click={() => {
            navigator.clipboard.writeText(
              `https://btcmap.org/merchant/${data.id}`,
            );
    ...
    {#if description}
      <p class="mx-auto max-w-[600px] text-primary dark:text-white">
        {description}
      </p>
    {/if}
          ...
          {#if merchantEvents && merchantEvents.length}
            ...
            {#if eventsPaginated.length !== merchantEvents.length}
              <button
                ...
                on:click={() => (eventCount = eventCount + 50)}
                >Load More</button
              >
            ...
            {#each filteredCommunities as community (community.id)}
              <div class="m-4 space-y-1 transition-transform hover:scale-110">
                <a href={resolve(`/community/${community.id}`)}>
                  ...
                  />
                  <p
                    class="text-center font-semibold text-body dark:text-white"
                  >
                    {community.tags.name}
                  </p>
                </a>
