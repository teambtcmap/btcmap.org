...
export const load: PageServerLoad<MerchantPageData> = async ({ params }) => {
	...
	try {
		...
		const boosted =
			...
			Date.parse(placeData.boosted_until) > Date.now()
				? placeData.boosted_until
				: undefined;
		...
		if (placeData.opening_hours)
			osmTags.opening_hours = placeData.opening_hours;
		...
		if (placeData["osm:survey:date"])
			osmTags["survey:date"] = placeData["osm:survey:date"];
		if (placeData["osm:check_date"])
			osmTags.check_date = placeData["osm:check_date"];
		if (placeData["osm:check_date:currency:XBT"])
			osmTags["check_date:currency:XBT"] =
				placeData["osm:check_date:currency:XBT"];
		...
		// Boost
		if (placeData.boosted_until)
			osmTags["boost:expires"] = placeData.boosted_until;
