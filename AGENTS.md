...
- Auth calls go straight to the btcmap API from the client with the Bearer token (the API answers CORS preflights; #1348). Don't add server-side proxies or website-side input validation for them — the API owns validation; server routes are reserved for flows that need a server-held secret (`/api/submit-place`, `/api/gitea/*`)
