...
- **Svelte 5, mixed-mode** — the app runs Svelte 5 with most components still in legacy (Svelte-4 syntax) mode. The table cluster (`src/routes/leaderboard`, `src/components/leaderboard/*`, `IssuesTable`, `ProfileActivity`) is runes-mode. Write NEW components in runes mode (`$props`, `$state`, `$derived`, `$effect`); leave existing legacy components as-is — incremental conversion is tracked in #1208, don't convert them as drive-by changes. Never use the removed imperative component API (`new Component()`/`$destroy`) — use `mount()`/`unmount()` from `svelte`.
