...
test.describe('Communities Page', () => {
	test('loads communities page successfully', async ({ page }) => {
		...
		// Check main heading
		await expect(page.getByRole('heading', { name: /join the bitcoin map community/i })).toBeVisible();
		
		// Check subtitle
		await expect(page.getByText(/take ownership of your local bitcoin mapping data/i)).toBeVisible();
	...
	test('handles organization sections', async ({ page }) => {
		...
		if (await sectionSelect.count() > 0) {
			...
			if (hasOrganizations) {
				...
				const organizationOptions = options.filter(option => 
					!option.includes('--') && 
					!['Africa', 'Asia', 'Europe', 'North America', 'Oceania', 'South America'].includes(option)
				);
