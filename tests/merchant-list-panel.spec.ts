...
test.describe("Merchant List Panel", () => {
  ...
  test("list panel opens via toggle button and shows merchants", async ({
    page,
  }) => {
    // Desktop viewport
    ...
    const listPanel = page.locator(
      '[role="complementary"][aria-label="Merchant list"]',
    );
    ...
    // Wait for merchant list items to load (indicates store is populated)
    const merchantItems = listPanel.locator("li button");
    await expect(merchantItems.first()).toBeVisible({ timeout: 15000 });

    // Should show "Nearby Merchants" heading (confirms we're in nearby mode, not search mode)
    await expect(
      listPanel.locator("h2:has-text('Nearby Merchants')"),
    ).toBeVisible({
      timeout: 5000,
    });
  });

  test("clicking merchant in list opens drawer with correct merchant", async ({
    page,
  }) => {
    // Desktop viewport
    await page.setViewportSize({ width: 1280, height: 720 });
    ...
    try {
      await page.waitForResponse(
        (response) =>
          response.url().includes("api.btcmap.org/v4/places/") &&
          response.status() === 200,
        { timeout: 10000 },
      );
    } catch (error) {
    ...
    const drawer = page.locator(
      '[role="dialog"]:has(a:has-text("View Full Details"))',
    );
  ...
  test("mobile: list panel opens fullscreen via toggle button", async ({
    page,
  }) => {
    // Mobile viewport
  ...
  test("mobile: selecting merchant closes list and opens drawer", async ({
    page,
  }) => {
    // Mobile viewport
    await page.setViewportSize({ width: 375, height: 667 });
